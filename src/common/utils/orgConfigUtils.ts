...
export async function restoreListViewMine(listViewStrings: Array<string>, conn: any, options: any = { debug: false }) {
    ...
    for (const listViewStr of listViewStrings) {
        ...
        if (splits.length === 2) {
            ...
            if (!m) {
                uxLog(this, c.red(`Unable to find list view object and name from ${listViewStr}. Use format ${c.bold("Object:ListViewName")} , or ${c.bold(".../objects/OBJECT/listViews/LISTVIEWNAME.listview-meta.xml")}`));
                continue;
            }
