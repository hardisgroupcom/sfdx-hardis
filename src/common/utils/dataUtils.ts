...
export async function selectDataWorkspace() {
    ...
    if (!fs.existsSync(dataFolderRoot)) {
      throw new SfdxError("There is no sfdmu root folder 'scripts/data' in your workspace. Create it and define sfdmu exports using sfdmu: https://help.sfdmu.com/")
    }

    const sfdmuFolders = fs.readdirSync(dataFolderRoot, { withFileTypes: true })
    ...
    const sfdmuDirResult = await prompts({
      ...
      choices: sfdmuFolders.map(sfdmuFolder => { return { title: sfdmuFolder, value: sfdmuFolder}})
    });
