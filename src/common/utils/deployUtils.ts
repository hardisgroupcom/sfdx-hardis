...
export async function forceSourcePush(scratchOrgAlias: string, debug = false) {
  try {
    ...
    await execCommand(pushCommand, this, {
      fail: true,
      output: true,
      debug: debug,
    });
    ...
    uxLog(
      this,
      c.yellow(
        tips.map((tip: any) => c.bold(tip.label) + "\n" + tip.tip).join("\n\n")
      )
    );
    uxLog(
      this,
      c.yellow(
        c.bold(
          `You may${
            tips.length > 0 ? " also" : ""
          } copy-paste errors on google to find how to solve the push issues :)`
        )
      )
    );
...
export async function forceSourcePull(scratchOrgAlias: string, debug = false) {
  try {
    ...
    const forceIgnoreElements = [
      ...(e.stdout + e.stderr).matchAll(
        /Entity of type '(.*)' named '(.*)' cannot be found/gm
      ),
    ];
    if (forceIgnoreElements.length > 0 && !isCI) {
      ...
      const forceIgnoreRes = await prompts({
        type: "multiselect",
        message:
          "If you want to try again with updated .forceignore file, please select elements you want to add",
        name: "value",
    ...
    uxLog(
      ...
      c.yellow(
        c.bold(
          `You may${
            tips.length > 0 ? " also" : ""
          } copy-paste errors on google to find how to solve the pull issues :)`
        )
      )
    );
...
export async function forceSourceDeploy(
  ...
  const splitDeployments = await buildDeploymentPackageXmls(
    packageXmlFile,
    check,
    debugMode
  );
  ...
  for (const deployment of splitDeployments) {
    ...
    if (deployment.waitBefore) {
      uxLog(
        commandThis,
        `Waiting ${deployment.waitBefore} seconds before deployment according to deployment plan`
      );
      await new Promise((resolve) =>
        setTimeout(resolve, deployment.waitBefore * 1000)
      );
    }
    ...
    if (deployment.packageXmlFile) {
      uxLog(
        commandThis,
        c.cyan(
          `Deploying ${c.bold(deployment.label)} package: ${
      ...
      if (deployRes.status === 0) {
        message = `[sfdx-hardis] Successfully ${
          check ? "checked deployment of" : "deployed"
        } sfdx project sources to Salesforce org`;
    ...
    if (deployment.waitAfter) {
      uxLog(
        commandThis,
        `Waiting ${deployment.waitAfter} seconds after deployment according to deployment plan`
      );
      await new Promise((resolve) =>
        setTimeout(resolve, deployment.waitAfter * 1000)
      );
    }
...
async function buildDeploymentPackageXmls(
  ...
  if (
    !(
      ...
      packageXml.Package.types.length > 0
    )
  ) {
  ...
  if (config.deploymentPlan && !check) {
    ...
    const mainPackageXmlCopyFileName = path.join(
      tmpDeployDir,
      "mainPackage.xml"
    );
    ...
    for (const deploymentItem of config.deploymentPlan.packages) {
      if (deploymentItem.packageXmlFile) {
        ...
        uxLog(
          this,
          c.cyan(
            `Removing ${deploymentItem.packageXmlFile} content from main package.xml`
          )
        );
...
export async function deployDestructiveChanges(
  ...
  uxLog(
    commandThis,
    c.cyan(
      `Deploying destructive changes from file ${path.resolve(
...
export async function deployMetadatas(
  ...
  try {
    ...
    if (JSON.stringify(e).includes("--soapdeploy")) {
      uxLog(
        this,
        c.yellow("This may be a error with a workaround... let's try it :)")
      );
      try {
        deployRes = await execCommand(
          deployCommand.replace(" --soapdeploy", ""),
          this,
        ...
        if (JSON.stringify(e2).includes("NoTestRun")) {
          ...
          uxLog(
            this,
            c.yellow(
              "This may be again an error with a workaround... let's make a last attempt :)"
            )
          );
          deployRes = await execCommand(
            deployCommand
              .replace(" --soapdeploy", "")
              .replace("NoTestRun", "RunLocalTests"),
            this,
            { output: true, debug: options.debug, fail: true }
          );
        } else {
...
async function replaceQuickActionsWithDummy() {
  if (process.env.CI_DEPLOY_QUICK_ACTIONS_DUMMY === "true") {
    uxLog(
      this,
      c.cyan(
        "Replacing QuickActions content with Dummy content that will always pass..."
      )
    );
    quickActionsBackUpFolder = await createTempDir();
    const patternQuickActions =
      process.cwd() +
      "/force-app/" +
      `**/quickActions/*__c.*.quickAction-meta.xml`;
    const matchQuickActions = await glob(patternQuickActions, {
      cwd: process.cwd(),
    });
  ...
  if (process.env.CI_DEPLOY_QUICK_ACTIONS_DUMMY === "true") {
    ...
    for (const quickActionFile of matchQuickActions) {
      const prevFileName = path
        ...
        .replace(
          path.resolve(quickActionsBackUpFolder),
