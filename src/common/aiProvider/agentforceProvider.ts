...
export class AgentforceProvider extends AiProviderRoot {
  ...
  private static async resolveConfig(): Promise<AgentforceResolvedConfig | null> {
    ...
    const agentforceConfig = projectConfig?.ai?.agentforce || {};
    const envEnabled = this.parseBoolean(getEnvVar("USE_AGENTFORCE"));
    const enabled = envEnabled ?? agentforceConfig.enabled ?? false;
    if (!enabled) {
      return null;
    }
    const promptTemplate = getEnvVar("GENERIC_AGENTFORCE_PROMPT_TEMPLATE") || agentforceConfig.promptTemplate || "SfdxHardisGenericPrompt";
    const promptUrlTemplate = getEnvVar("GENERIC_AGENTFORCE_PROMPT_URL") || agentforceConfig.promptUrl || `/services/data/v{{API_VERSION}}/einstein/prompt-templates/{{PROMPT_TEMPLATE}}/generations`;
