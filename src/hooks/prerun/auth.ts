...
export const hook = async (options: any) => {
  ...
  if (((options?.Command?.requiresUsername === true) || options.checkAuth === true) && !(options.devHub === true)) {
    if (options?.Command?.requiresUsername === true && process.argv.includes("--skipauth")) {
      return ;
    }
