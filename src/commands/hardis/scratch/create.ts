...
export default class ScratchCreate extends SfdxCommand {
  ...
  public async createScratchOrg() {
    ...
    assert(
      ...
      this.buildScratchCreateErrorMessage(createResult)
    );
  ...
  public buildScratchCreateErrorMessage(createResult) {
    if (createResult.status === 0 && createResult.result) {
      ...
      return c.red(
        `[sfdx-hardis] Error creating scratch org. ${c.bold("This is probably a Salesforce error, try again manually or launch again CI job")}\n${JSON.stringify(
          createResult,
          null,
          2
        )}`
