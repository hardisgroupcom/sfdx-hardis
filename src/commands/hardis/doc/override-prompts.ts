...
- **File Copying:** Iterates through predefined \`PROMPT_TEMPLATES\` and \`PROMPT_VARIABLES\` objects. For each template/variable, it extracts the English text content and writes it to a corresponding \`.md\` file in the \`config/prompt-templates/\` directory.
...
`;
  ...
  // Copy all prompt templates as .md files
  for(const [templateName, templateDefinition] of Object.entries(PROMPT_TEMPLATES)) {
  const targetFile = path.join(promptTemplatesDir, `${templateName}.md`);
  const legacyFile = path.join(promptTemplatesDir, `${templateName}.txt`);
  ...
  if (fs.existsSync(targetFile) || fs.existsSync(legacyFile)) {
    if (flags.overwrite) {
      ...
      // Overwrite the existing file (always write to .md)
  ...
  // Write the prompt text to the .md file
  fs.writeFileSync(targetFile, promptText);
  uxLog("success", this, c.green(t('createdTxt', { templateName })));
  createdCount++;
}

// Copy all prompt variables as .md files
for (const [variableName, variableDefinition] of Object.entries(PROMPT_VARIABLES)) {
  const targetFile = path.join(promptTemplatesDir, `${variableName}.md`);
  const legacyFile = path.join(promptTemplatesDir, `${variableName}.txt`);
  ...
  // Write the variable text to the .md file
  fs.writeFileSync(targetFile, variableText);
...
const usageMessage = [
  ...
  '',
  'Usage:',
  '   - Edit template .md files to customize AI prompts',
  '   - Edit variable .md files to customize common instruction patterns',
