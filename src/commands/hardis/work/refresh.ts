...
export default class RefreshTask extends SfdxCommand {
    ...
    protected static flagsConfig = {
        nopull: flags.boolean({ char: 'n', default: false, description: "No scratch pull before save (careful if you use that!)" }),
    ...
    public async run(): Promise<AnyJson> {
        ...
        const branchChoices = [{ title: `${config.developmentBranch} (recommended)`, value: config.developmentBranch }];
    ...
    private async runRefresh(localBranch): Promise<AnyJson> {
        ...
        uxLog(this, c.cyan(`sfdx-hardis will refresh your local branch ${c.green(localBranch)} and your local scratch org ${c.green(this.org.getUsername())} with the latest state of ${c.green(this.mergeBranch)}`));
        ...
        // Stash
        uxLog(this, c.cyan(`Stashing your uncommited updates in ${c.green(localBranch)} before merging ${c.green(this.mergeBranch)} into your local branch ${c.green(localBranch)}...`));
