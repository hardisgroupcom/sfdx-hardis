...
- **Users with the MFA-bypass permission.** Queries Profiles and Permission Sets where \`PermissionsBypassMFAForUiLogins = true\` and lists every active Standard user assigned to them. The field is not provisioned in every org edition, so its existence is verified with a describe call first; when absent, the check passes with an explanation instead of crashing.
