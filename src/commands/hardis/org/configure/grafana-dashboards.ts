...
- **Guided start:** in interactive mode, the command first asks whether a Grafana instance is already available. If not, it opens the [Grafana free-tier setup guide](${CONSTANTS.DOC_URL_ROOT}/salesforce-devops-setup-integration-api/#grafana-setup) (Grafana Cloud free tier: free forever, no credit card required) instead of asking for a URL.
...
`;
  ...
  public static flags: any = {
    'grafana-url': Flags.string({
      description: 'Grafana instance URL (defaults to GRAFANA_API_URL environment variable, then to grafanaUrl in .sfdx-hardis.yml)',
    }),
    ...
    'prom-uid': Flags.string({
      description: 'Uid of the Prometheus/Mimir datasource used by the alert pack (--with-alerts only, auto-detected when not set)',
    }),
    'loki-uid': Flags.string({
      description: 'Uid of the Loki datasource used by the alert pack (--with-alerts only, auto-detected when not set)',
    }),
  ...
  public async run(): Promise<AnyJson> {
    ...
    if (!isCI && !this.agentMode) {
      ...
      if (hasGrafana.value !== true) {
        const docUrl = `${CONSTANTS.DOC_URL_ROOT}/salesforce-devops-setup-integration-api/#grafana-setup`;
    ...
    // Non-secret Grafana settings can live in .sfdx-hardis.yml; the token never does
    ...
    try {
      grafanaUrl = normalizeGrafanaUrl(rawUrl);
    } catch {
      throw new SfError(t('errorGrafanaUrlInvalid', { url: rawUrl }));
    }
    ...
    try {
      ...
      throw this.grafanaApiError('POST /api/folders', e);
    }
    ...
    for (const file of files) {
      let dashboard: any;
      try {
        const rawJson = await fetchRawUrl(file.downloadUrl);
      ...
      try {
        ...
        throw this.grafanaApiError(`import ${file.name}`, e);
      }
    }

    // Alert pack (optional). Alert rules need explicit datasource uids, so detection
    // happens only here: a plain dashboard install works without any datasource setup.
    let importedAlertRules = 0;
    let promUid: string | null = null;
    let lokiUid: string | null = null;
    if (flags['with-alerts'] === true) {
      ...
      try {
        ...
        groups = buildAlertRuleGroups(alertsYaml, promUid, lokiUid, GRAFANA_V2_FOLDER_UID);
      } catch (e: any) {
      ...
      try {
        ...
        throw this.grafanaApiError('import alert rules', e);
      }
      uxLog("log", this, c.grey(t('grafanaAlertsImported', { total: importedAlertRules })));
    }
    ...
    uxLog("success", this, c.green(t('grafanaInstallSuccess', { total: importedDashboards.length, url: fleetUrl })));
  ...
  private grafanaApiError(operation: string, e: any): SfError {
    const message = t('errorGrafanaApi', { operation: operation, message: grafanaApiErrorMessage(e) });
    return new SfError(isGrafanaAuthError(e) ? `${message}\n${t('errorGrafanaAuthHint')}` : message);
  }
