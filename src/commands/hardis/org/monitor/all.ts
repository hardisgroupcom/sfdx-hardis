...
- **Weekly PPTX report:** On weekly runs (Saturday, or when \`MONITORING_IGNORE_FREQUENCY=true\`), a **PowerPoint report** is generated by a [coding agent](${CONSTANTS.DOC_URL_ROOT}/salesforce-deployment-agent-autofix/) (Claude, Codex, Gemini, or Copilot) and attached to the summary notification. Requires \`codingAgent\` to be configured.
...
The command runs each monitoring sub-command sequentially and collects exit codes.
...
`;
  ...
  public async run(): Promise<AnyJson> {
    ...
    if (aiAvailable) {
      try {
        const notifications = await collectMonitoringNotifications(notifDir);
        if (notifications.length > 0) {
          // Generate AI summary
          const aiSummary = await generateMonitoringAiSummary(notifications, orgUrl);
          ...
          // Generate PPTX on weekly (Saturday) runs using a coding agent
          ...
          if (isWeekly) {
            try {
              ...
              if (pptxPath) {
                summaryNotif.attachedFiles = [pptxPath];
              }
            } catch (pptxErr) {
