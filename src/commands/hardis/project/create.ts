...
export default class ProjectCreate extends SfdxCommand {
  ...
  public async run(): Promise<AnyJson> {
    ...
    // Connect to DevHub
    await this.config.runHook('auth', { checkAuth: true, Command: this, devHub: true, scratch: false });
    // Project name
    ...
    const createCommand = 'sfdx force:project:create'+
       ` --projectname "${projectName}"`+
       ' --manifest';
    await execCommand(createCommand,this,{output:true,fail:true,debug: this.debugMode});

    // Move project files at root
    await fs.copy(path.join(process.cwd(), projectName), process.cwd(), { overwrite: false });
    ...
    uxLog(this,c.cyan("SFDX Project has been created. You can now customize config/project-scratch-def.json file to add required features and settings.\nhttps://developer.salesforce.com/docs/atlas.en-us.sfdx_dev.meta/sfdx_dev/sfdx_dev_scratch_orgs_def_file.htm"));
