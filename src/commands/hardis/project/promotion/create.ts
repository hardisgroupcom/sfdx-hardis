...
`;
  ...
  public async run(): Promise<AnyJson> {
    ...
    const result = await pushAndCreatePromotionPullRequest({
      ...
      body,
      commandThis: this,
      skipPullRequest: flags['skip-pull-request'] === true,
    });
    ...
    if (conflicted.length > 0) {
      uxLog('warning', this, c.yellow(t('promotionCreateDoneWithConflicts', {
      ...
      conflictPromptFile = await writeConflictResolutionPrompt({
        ...
        targetBranch,
        branchName,
        conflicted: stories.filter((story) => (story.conflictFiles || []).length > 0),
