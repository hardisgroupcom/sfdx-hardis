...
export default class PackageVersionPromote extends SfdxCommand {
  ...
  public async run(): Promise<AnyJson> {
    ...
    if (auto) {
      ...
      const packageListRes = await execSfdxJson("sfdx package:version:list --released", this, { output: true, fail: true });
    ...
    for (const packageToPromote of packagesToPromote) {
      ...
      const promoteCommand = "sfdx package:version:promote" + ` --package "${packageToPromote}"` + " --noprompt";
