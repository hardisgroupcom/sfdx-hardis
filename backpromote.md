...
- R22. Items listed in `package-no-overwrite.xml` (or `packageDeployOnce.xml`) of the parent branch are compared with the sandbox like any other item and carry a `package-no-overwrite.xml` marker. When the item is absent from the sandbox (every file of it is `missingInOrg`) it is ticked by default and deployed like any other item; unticking it records it as left out (`excluded`). When the sandbox already has it (at least one of its files was found there, or none of its files could be compared) it is unticked by default, not deployed and not recorded as left out. The user can tick it: it is then passed with `--include-no-overwrite Type:Name` (repeatable) and treated like any other item, with the Overwrite / Keep org version / Merge decision when it differs. Every other item and every deletion is ticked by default.
...
- R38. One coding agent prompt per run, never per file: it lists every prepared file with its three versions and the rule to solve each, asks the agent to solve the markers in the checkout of the backpromote branch and to commit the solved files on that branch with a message body naming each file, then gives the command to run next (a file the agent left uncommitted is still committed by the run). It is written in `hardis-report/` and its path is returned in the JSON, as for promotion branches (`buildBackpromoteMergePrompt` is kept and reworded). It is rewritten each time `--prepare` adds a file.
...
- R53. **A merge decided by the agent is a normal file edit.** In agent mode, `--on-diff file=merge` writes the merged file with markers in the checkout of the backpromote branch, then the run stops with status `waitingForMerges` (exit code 0, it is not an error), the JSON lists the files with their absolute paths, and the coding agent prompt of R38 contains the exact command to run again. The agent edits and commits the files on the backpromote branch, then runs the same command with the `runId`: it checks the markers, commits what is still uncommitted, deploys. A file left with markers is reported, not deployed, and the command can be run again after another edit. An agent can also use `--prepare` like the panel and run once.
...
- `items[]`: key, type, name, files, Pull Requests, `excludedLastTime`, `noOverwrite` (whether the item is in the sandbox, which decides if it is ticked by default, is read from its `comparison[]` entries, see R22)
- `deletions[]`: key, files
...
8. **`package-no-overwrite.xml`** applies to dev sandboxes too: an item already in the sandbox is unticked by default, one absent from it is ticked (R22).
