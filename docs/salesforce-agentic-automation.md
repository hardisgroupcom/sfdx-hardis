...
| [**hardis:project:promotion:list-candidates**](hardis/project/promotion/list-candidates.md) | List the User Stories that could be promoted from a major branch, read-only, with `--source-branch` and `--json`. Creates and closes nothing (experimental)                         |
| [**hardis:project:promotion:create**](hardis/project/promotion/create.md)                   | Assemble a promotion branch carrying only the chosen User Stories, and open its Pull Request, with `--agent --source-branch --pull-requests` (experimental)                         |
