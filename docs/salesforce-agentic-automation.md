...
| [**hardis:ticket:test-cases:upsert**](hardis/ticket/test-cases/upsert.md) | Send the test cases of a notebook to Azure DevOps, ServiceNow or Xray. `--agent` skips the confirmation asked before any write. Required: `--notebook` or `--testsjsonfile`; optional: `--provider`, `--dry-run` |
