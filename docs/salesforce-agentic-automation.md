...
| [**hardis:packagexml:remove-managed**](hardis/packagexml/remove-managed.md)                       | Strip all managed-package items from a `package.xml`, preserving custom metadata on managed objects - namespaces auto-detected or supplied via `--namespaces`; strategy selectable with `--namespace-detection` |
