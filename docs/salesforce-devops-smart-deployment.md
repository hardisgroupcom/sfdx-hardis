...
The command [`sf hardis:project:deploy:smart`](hardis/project/deploy/smart.md) is the central deployment engine of [sfdx-hardis CI/CD](salesforce-devops-home.md). It orchestrates a pipeline that handles **[delta deployments](salesforce-devops-config-delta-deployment.md)**, **[Quick Deploy](https://developer.salesforce.com/docs/atlas.en-us.api_meta.meta/api_meta/meta_quick_deploy.htm)**, **test class selection**, **[package installation](salesforce-devops-work-on-user-story-install-packages.md)**, **[overwrite management](salesforce-devops-config-overwrite.md)**, **[pre/post commands](salesforce-devops-work-on-user-story-deployment-actions.md)**, and **[notifications](salesforce-devops-setup-integrations-home.md)** in a single flow.
...
The command reads its configuration from `config/.sfdx-hardis.yml` (or branch-specific files like `config/branches/.sfdx-hardis-BRANCHNAME.yml`). See [CI/CD Configuration](salesforce-devops-config-home.md) for details. Key flags include:
...
Delta deployment reduces the deployment scope to only the metadata that changed between the source and target branches, using [sfdx-git-delta](https://github.com/scolladon/sfdx-git-delta). See [Delta Deployment Configuration](salesforce-devops-config-delta-deployment.md) for setup instructions.
...
Pull Request descriptions can also override config properties using YAML blocks (see [Deployment Actions](salesforce-devops-work-on-user-story-deployment-actions.md)):
...
- Deployment errors (with [fix tips](salesforce-devops-solve-deployment-errors.md))
- [Code coverage](https://developer.salesforce.com/docs/atlas.en-us.apexcode.meta/apexcode/apex_code_coverage_intro.htm) report
- [Pre/post command](salesforce-devops-work-on-user-story-deployment-actions.md) results
- Commit summary
