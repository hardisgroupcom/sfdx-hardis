...
This checklist follows the [Setup Guide](salesforce-devops-setup-home.md) in chronological order, with the [initialization Pull Request](salesforce-devops-setup-merge-request.md) (Merge Request on GitLab) as the pivot: some items must be done **before** it, others only make sense **after** it.
...
_See [Create the Git repository](salesforce-devops-setup-git.md)_

- [ ] The repository exists and contains the sfdx-hardis project sources.
- [ ] The [branch pattern](salesforce-devops-setup-git.md#create-the-major-branches) is decided and written down: **pattern A** (BUILD and hotfixes) or **pattern B** (BUILD, RUN and hotfixes, with a `uat_run` branch).
...
- [ ] Contributors know where an urgent fix goes: a [hotfix](salesforce-devops-hotfixes.md) branch merges into `preprod`, never into a major org by hand, and it is [retrofitted](salesforce-devops-retrofit.md) into the BUILD right after.

#### Salesforce orgs

_See [Prepare the Salesforce orgs](salesforce-devops-setup-activate-org.md)_
...
_See [Initialize the SFDX project](salesforce-devops-setup-init-project.md)_
...
- [ ] Installed packages are listed in `config/.sfdx-hardis.yml`, with `installDuringDeployments` and `installOnScratchOrgs` set according to your needs. _See [Retrieve installed packages](salesforce-devops-setup-existing-org.md#retrieve-installed-packages)_
...
_See [Configure CI authentication](salesforce-devops-setup-auth.md)_
...
- [ ] `SFDX_CLIENT_KEY_<ALIAS>` (AES passphrase) and/or `SFDX_CLIENT_CERT_<ALIAS>` is defined, matching the [certificate storage mode](salesforce-devops-setup-auth.md#certificate-storage-modes) you selected.
...
- **Azure DevOps / Azure Pipelines** _(see [variables](salesforce-devops-setup-auth-azure.md), [integration](salesforce-devops-setup-integration-azure.md))_
...
- **Jenkins** _(see [Jenkins setup](salesforce-devops-setup-auth-jenkins.md))_
...
_See [Project configuration guide](salesforce-devops-config-home.md) and the [full list of configuration properties](schema/sfdx-hardis-json-schema-parameters.html)_

**Overwrite management is the most important part of this section.** Without `manifest/package-no-overwrite.xml`, every deployment overwrites the metadata that is maintained directly in the orgs: business users lose their Reports and Dashboards, and org-specific credentials and URLs are replaced by the ones of another environment. _See [Overwrite management](salesforce-devops-config-overwrite.md)_
...
_Overview: [Configure integrations](salesforce-devops-setup-integrations-home.md)_
...
- **Slack** _(see [Slack integration](salesforce-devops-setup-integration-slack.md))_
...
- **Microsoft Teams** _(see [Teams integration](salesforce-devops-setup-integration-ms-teams.md))_
...
- **Google Chat** _(see [Google Chat integration](salesforce-devops-setup-integration-google-chat.md))_
...
- **Email** _(see [Email integration](salesforce-devops-setup-integration-email.md))_
...
- **API, for example Grafana** _(see [API integration](salesforce-devops-setup-integration-api.md))_
...
- **Jira** _(see [Jira integration](salesforce-devops-setup-integration-jira.md))_
...
_See [First Pull Request](salesforce-devops-setup-merge-request.md)_
...
- [ ] The **check deploy** job is triggered by the Pull Request, and ends green. _See [Solve deployment errors](salesforce-devops-solve-deployment-errors.md)_
- [ ] The **code quality** (MegaLinter) job is green, or its remaining errors are known and accepted. _See [Handle MegaLinter errors](salesforce-devops-solve-megalinter-errors.md)_
...
- [ ] Quick Deploy is effective: after a successful validation job, the deployment job reuses the validated deployment instead of running a full one (`SFDX_HARDIS_QUICK_DEPLOY` is not set to `false`). _See [Smart Deployments](salesforce-devops-smart-deployment.md)_
...
_See [Delta deployments](salesforce-devops-config-delta-deployment.md)_
...
_See [Prepare the Salesforce orgs](salesforce-devops-setup-activate-org.md) for the recommended sandbox types_
...
- [ ] The External Client App of the **production** org is in place, created manually if Apex test errors prevented the automated deployment. _See [Configure CI authentication](salesforce-devops-setup-auth.md)_
...
- [ ] Leftover translations of deleted Dashboards and Reports are removed from `translations/*.xml`. _See [Common issues](salesforce-devops-setup-merge-request.md#common-issues)_
- [ ] No credential, token or Salesforce URL with a session is present in the git history.

#### Team onboarding

- [ ] Every contributor installed the required tooling and the VS Code SFDX Hardis extension. _See [Installation guide](salesforce-devops-use-install.md)_
- [ ] Every contributor cloned the repository and can create a User Story branch. _See [Clone repository](salesforce-devops-clone-repository.md)_
- [ ] The team knows the [contribution process](salesforce-devops-use-home.md): create a User Story, work on it, publish it, handle Pull Request results.
- [ ] Release managers know how to [review and merge Pull Requests](salesforce-devops-validate-merge-request.md) and how to handle [hotfixes](salesforce-devops-hotfixes.md) and their [retrofit](salesforce-devops-retrofit.md).
