...
| include-promotions   | boolean | With promotion branches (beta) enabled, also list the promotion Pull Requests next to the User Stories they carry (left out by default, the stories are what the release delivers) |         |          |                  |
