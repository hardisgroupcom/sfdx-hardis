...
| loki-uid      | option  | Uid of the Loki datasource receiving sfdx-hardis logs (auto-detected when not set)                  |         |          |         |
| prom-uid      | option  | Uid of the Prometheus/Mimir datasource receiving sfdx-hardis metrics (auto-detected when not set)   |         |          |         |
