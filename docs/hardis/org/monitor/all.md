...
- **Weekly PPTX report:** On weekly runs (Saturday, or when `MONITORING_IGNORE_FREQUENCY=true`), a **PowerPoint report** can be generated by a [coding agent](https://sfdx-hardis.cloudity.com/salesforce-deployment-agent-autofix/) (Claude, Codex, Gemini, or Copilot) and attached to the summary notification.
