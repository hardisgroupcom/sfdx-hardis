...
> If your colleagues merged changes in the meantime, [Backpromote (Beta)](hardis/work/backpromote.md) brings them into your dev sandbox or scratch org so you work on an up-to-date org. In VS Code, the **Backpromote (Beta)** panel lists the Pull Requests waiting, lets you pick them one by one, shows the items changed in your org (compare, keep, or merge them with a ready-to-paste coding agent prompt), then deploys in one click. What each org already received is recorded in the Pull Request comments and shared with your team, so Backpromote needs a connection to your git provider (GitHub, GitLab, Azure DevOps or Bitbucket).
