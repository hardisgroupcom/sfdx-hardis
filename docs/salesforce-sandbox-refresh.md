...
The actions report is cumulative across runs: it contains everything taken into account by all runs for the sandbox (with a run date on each line). Rows from sections that clean-replace their data (certificates, custom settings) are replaced when the section is re-executed; rows describing backup content still on disk are kept. The history is written to disk after every action, so a run interrupted before the end (killed process, cancelled prompt) still leaves everything done so far in the next report.
