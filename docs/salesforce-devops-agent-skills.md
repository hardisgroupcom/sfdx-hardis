...
|--------------------------|---------|---------------------------------------------------------------------------------------------------|
| `--pr-id`                |         | Pull Request ID (for `pr` scope). `current` auto-detects from branch.                             |
...
| `--run-only-once-by-org` | `true`  | Execute only once per target org (state tracked in the "Deployment Actions" Pull Request comment) |
...
| `hardis:project:promotion:list-candidates` | Lists, read-only, the User Stories that could be promoted from a major branch. Creates, pushes and closes nothing, so it is the safe one to call first (experimental)                                                                        | [Promotion branches](salesforce-devops-promotion-branches.md) |
| `hardis:project:promotion:create`          | Assembles a promotion branch carrying only the chosen User Stories and opens its Pull Request (experimental)                                                                                                                                 | [Promotion branches](salesforce-devops-promotion-branches.md) |
