...
- The delta package is displayed **before** overwrite filtering: [package-no-overwrite.xml](salesforce-devops-config-overwrite.md) can still remove protected items from it.
...
- **ProfilePermissionProcessor**: Makes sure profile and permission set updates are deployed when related metadata changes.
...
- **LayoutProcessor**: Makes sure layout changes are deployed when fields or objects are updated.
...
- For deletions, the feature also tries to include related translations and entries so removing something in a delta does not break a future full deployment.

Practical examples:

- You remove a picklist value in a feature branch. The delta will include any Record Types and translations that reference that value so later full deployments do not fail.
...
- Start by enabling the feature on a non-critical branch (or for a single Pull Request) to confirm the produced delta includes the expected additional metadata before rolling it out broadly.

Note: this feature is in beta. It helps reduce deployment surprises, but test it on your repository and workflows before relying on it for critical releases.
...
In case of temporary deactivation of delta deployments, you can set variable `DISABLE_DELTA_DEPLOYMENT=true`, it has priority over the other configurations.
...
It is recommended to keep the opinionated default sfdx-hardis delta deployment configuration, but if you want to tweak it, you can use the variables below.
...
> Standard sfdx-hardis pipeline does not recommend to use these modes, but if you really know what you are doing, like the artists of [BeyondTheCloud.dev](https://blog.beyondthecloud.dev/for-developers), you can use them.

- USE_DELTA_DEPLOYMENT_AFTER_MERGE
  - By default, after a merge sfdx-hardis will try to use [QuickDeploy](salesforce-devops-setup-integrations-home.md#git-providers). If not available, it will perform a full deployment. If you want to use a delta deployment anyway, define `USE_DELTA_DEPLOYMENT_AFTER_MERGE=true`
