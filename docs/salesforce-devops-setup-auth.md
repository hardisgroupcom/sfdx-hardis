...
description: With sfdx-hardis, learn how to configure CI Server authentication to automate deployments
---
...
To automate [deployments from major branches to their related org](salesforce-devops-deploy-major-branches.md), you need to **configure secure authentication from your CI server to an External Client App**.
...
> If messages ask you to **run the same command twice**, it is **normal**: it is for technical reasons.

> If you have **errors in your Apex test classes**, you may not be able to deploy the app to the Production org automatically.
...
> You can do it later, after having succeeded to merge the first Pull Request (Merge Request on GitLab) in the lowest major branch (usually `integration`).
...
- [GitLab tutorial](salesforce-devops-setup-auth-gitlab.md)
- [Azure tutorial](salesforce-devops-setup-auth-azure.md), **with Pipeline setup instructions**
- [GitHub tutorial](salesforce-devops-setup-auth-github.md)
- [Bitbucket tutorial](salesforce-devops-setup-auth-bitbucket.md)
- [Jenkins tutorial](salesforce-devops-setup-auth-jenkins.md)
...
> To learn how sfdx-hardis resolves credentials at runtime (alias lookup order, JWT private key format auto-detection, key file lookup paths, etc.), see the **Technical explanations** section of [`hardis:auth:login`](hardis/auth/login.md).
