...
| `GITHUB_TOKEN` + `CI_SFDX_HARDIS_GITHUB_TOKEN` / `CI_SFDX_HARDIS_GITLAB_TOKEN` / `CI_SFDX_HARDIS_BITBUCKET_TOKEN` / `CI_SFDX_HARDIS_AZURE_TOKEN` | Git provider integration: post deployment comments on PRs/MRs and allow the coding agent to push fix branches and open PRs. See [Git provider integration from Jenkins](#git-provider-integration) |
