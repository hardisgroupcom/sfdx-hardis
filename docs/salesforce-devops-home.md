...
description: With sfdx-hardis, run a production-grade Salesforce CI/CD pipeline on your own Git platform, with a visual workflow for admins, smart deployments, deployment actions, release notes and DORA metrics
---
...
|----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Admins**           | [Build Pull Requests](salesforce-devops-publish-user-story.md) from the **VS Code extension** with clicks. No command line.                                                |
...
1. [**Start a User Story**](salesforce-devops-create-new-user-story.md): a Git branch is created and a dev sandbox or scratch org is assigned to it.
2. [**Work in your org**](salesforce-devops-work-on-user-story.md): configure and develop in Salesforce Setup or VS Code, as usual.
3. [**Publish your User Story**](salesforce-devops-publish-user-story.md): retrieve your changes with the Metadata Retriever, commit the ones you want to publish, then Save / Publish. The manifest is updated, the sources are cleaned, the commit is pushed and the Pull Request is created.
4. [**Check the Pull Request**](salesforce-devops-handle-merge-request-results.md): the CI server simulates the deployment, runs the Apex tests and the quality checks, and posts the results as comments on the Pull Request.
5. [**Review, merge and deploy**](salesforce-devops-release-home.md): once merged, the CI server deploys to the matching org, runs the deployment actions, notifies the team and updates the tickets.
...
- [Delta deployments](salesforce-devops-config-delta-deployment.md): deploy only what changed.
- [Overwrite management](salesforce-devops-config-overwrite.md): protect metadata that must never be overwritten by the pipeline.
...
- [Automated source cleaning](salesforce-devops-config-cleaning.md): tidy profiles, flow positions and more before each Pull Request.
- [Quick Deploy](salesforce-devops-smart-deployment.md): reuse the validated deployment after the merge instead of deploying twice.

**Deployment actions**

Deploying a User Story is not always just about metadata. With [deployment actions](salesforce-devops-work-on-user-story-deployment-actions.md), contributors declare on their Pull Request what must happen before or after the deployment: run a command, import data with SFDMU, run an Apex script, publish an Experience Cloud site, schedule an Apex batch, remove items from package.xml, or remind a human to do something in Setup. sfdx-hardis runs them in every org the User Story reaches, once per org, and tracks what has been done in a Pull Request comment.

![Deployment actions of a Pull Request](assets/images/screenshot-pr-deployment-actions-list.jpg)

**Releases and reporting**

- [Release Notes](hardis/doc/salesforce-devops-release-notes.md): generated from the Git history, the tickets, the metadata changes and the deployment actions. Markdown, PDF and XLSX outputs, with an optional AI summary.
- [DORA Metrics](hardis/doc/salesforce-devops-dora-report.md): Deployment Frequency, Lead Time for Changes, Change Failure Rate and MTTR, scored Elite / High / Medium / Low against industry benchmarks.
- [Promotion branches (experimental)](salesforce-devops-promotion-branches.md): when business signs off story by story and only some of `uat` is approved, ship the approved ones alone. The release manager ticks them in the DevOps Pipeline, the branch is assembled by cherry-pick, and its Pull Request declares what it carries so the deployment actions, Apex test classes and tickets of those stories follow them.
- [Backpromote (Beta)](salesforce-devops-backpromote.md): bring what was merged in a parent branch into a developer's sandbox, with the deployment actions of the merged Pull Requests, a decision for each file that differs in the sandbox, and a history shared through the Pull Request comments.
...
See [Work with AI coding agents](salesforce-devops-agent-skills.md) for the full picture.
...
- **Per-channel notifications** routed independently to [Slack and Microsoft Teams](salesforce-devops-setup-integration-slack.md), [email](salesforce-devops-setup-integration-email.md) and [API / Grafana / Prometheus](salesforce-devops-setup-integration-api.md), with a severity threshold per notification type (stream everything to Grafana, keep Slack for warnings and errors).
...
Open-source and free. The [**Setup Guide**](salesforce-devops-setup-home.md) walks you through the initialization of a Salesforce CI/CD project, from the Git repository to the first Pull Request.
...
- [**Contributor Guide**](salesforce-devops-use-home.md): work on a CI/CD project as a business analyst, admin or developer.
- [**Release Manager Guide**](salesforce-devops-release-home.md): review Pull Requests, drive deployments and releases, configure the project.
- [**Setup Guide**](salesforce-devops-setup-home.md): initialize a Salesforce CI/CD project from scratch.
