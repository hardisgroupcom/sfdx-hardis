...
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): bring what your teammates merged in the parent branch into your User Story branch and your org in one go: your pending org changes are saved first, the parent branch is merged (each conflicting file gets one decision: overwrite, keep, or merge by hand with a coding agent prompt), then what the merge brought in is deployed with its deletions, Apex test classes and deployment actions. It replaces `hardis:work:refresh`, and the new VS Code Backpromote (Beta) panel runs it.
