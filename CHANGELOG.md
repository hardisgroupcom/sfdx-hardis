...
- [Custom commands](https://sfdx-hardis.cloudity.com/salesforce-ci-cd-config-home/): new optional `showCommandDetails` on a custom command, so the VS Code command execution panel opens with Advanced details already on. The panel also shows the command's label instead of its command line, with the command line as the tooltip.
