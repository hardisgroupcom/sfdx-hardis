...
- [hardis:work:new](https://sfdx-hardis.cloudity.com/hardis/work/new/): updating an existing sandbox no longer deploys the metadata of the target branch into it, and the command points to [backpromote](https://sfdx-hardis.cloudity.com/salesforce-ci-cd-backpromote/) for that.
- Git providers and ticketing providers: every batch of parallel calls (merged Pull Requests per branch, Deployment Actions comments, Backpromotes comments, JIRA, Azure Boards and ServiceNow ticket details) leaves in batches sized on what the provider serves concurrently (80 at a time on GitHub and GitLab, 50 on Azure DevOps and Bitbucket, 40 on Jira Server, 20 on Jira Cloud, 8 on ServiceNow) and, only when the provider throttles, waits the delay it asks for and goes on with smaller batches.
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): bring what your teammates merged in a parent branch (integration, uat...) into your developer sandbox, with the deployment actions of the merged Pull Requests: the history of each sandbox is read from a "Backpromotes" comment on every Pull Request (so several people can backpromote to the same sandbox from different computers), each file that differs in the sandbox gets one decision (overwrite, keep the org version, or merge by hand with the VS Code merge editor or a coding agent prompt), and the deployment runs from a `backpromote/<parent branch>/<sandbox>` branch. It replaces `hardis:work:refresh`, and the new VS Code Backpromote (Beta) panel runs it.
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): a static resource that exists in the sandbox is no longer reported as absent from it and overwritten without a question, and a merge by hand now shows the version both sides started from.
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): the uncommitted changes are stashed even when the repository holds a folder Windows cannot open (a name ending with a dot).
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): a plan is computed in seconds instead of minutes on a large repository.
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): the sandbox files of the comparison are reused from one plan to the next when the org shows they did not change.
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): an item of `package-no-overwrite.xml` is now deployed when it is not in the sandbox yet, and one already there can be deployed on request with `--include-no-overwrite`.
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): the coding agent prompt now asks the agent to commit the files it merged on the backpromote branch, which the new Merge all button of the VS Code panel copies for every differing item at once.
- Git providers: a Pull Request comment written by marker is now searched across every page of comments before being created, so a busy Pull Request never gets it twice, and a git provider whose listing fails no longer passes as an empty one.
...
- [hardis:project:promotion:create](https://sfdx-hardis.cloudity.com/hardis/project/promotion/create/): a promotion undone after a cherry-pick conflict no longer ends on an error line saying the branch it just deleted was not found.
- [hardis:project:promotion:create](https://sfdx-hardis.cloudity.com/hardis/project/promotion/create/): on GitLab, the merge request number of each candidate is now read from the merge commit even when GitLab cannot be reached, so a story can still be selected by its number.
...
- Fixed the documentation site failing to build with Zensical on projects whose menus were generated by an earlier version: sub-menus were written as a single block, and Zensical stops on it with "Unknown nav item value type". Your `mkdocs.yml` is repaired the next time you run a doc command, and any menu you added by hand is kept as it is.
- [hardis:doc:project2markdown](https://sfdx-hardis.cloudity.com/hardis/doc/project2markdown/): Apex classes carrying a `@group` tag now get their documentation page, instead of being left out with dead links pointing at them.
- [hardis:doc:project2markdown](https://sfdx-hardis.cloudity.com/hardis/doc/project2markdown/): Generated pages no longer link to pages that do not exist, so the site builds without "page does not exist" warnings.
- [hardis:doc:project2markdown](https://sfdx-hardis.cloudity.com/hardis/doc/project2markdown/): Documentation menus are now sorted alphabetically, so entries like Lightning Web Components and packages no longer come out in file system order.
...
- When run from VS Code, a command now loads its implementation while waiting for the extension's go-ahead instead of after it: with extension versions that answer late (or never), the first prompt shows up to 3 seconds earlier. The matching extension fix removes the remaining 10-second wait.
...
- **Release notes: Pull Requests and Commits columns in the Metadata Changes XLSX tab**: each metadata item of [hardis:doc:release-notes](https://sfdx-hardis.cloudity.com/hardis/doc/release-notes/) now lists the Pull Requests (number, title, author) and the commits (title, author, date) that touched it, one per line.
...
- [hardis:project:clean:hiddenitems](https://sfdx-hardis.cloudity.com/hardis/project/clean/hiddenitems/): Clean hidden managed package sources and handle directories, not only files.
...
- External Client Apps satellite metadatas: Fixed the hardcoded suffixes to query actual names from the org
...
- New deployment action management commands with full `--agent` support for non-interactive execution:
  ...
  - Selective Apex Tests management
...
- Allow DevOps command to be call by headless agents using **--agent**
  ...
  - [hardis:work:save](https://sfdx-hardis.cloudity.com/hardis/work/save/)
- Allow to send auth headers to enterprise OpenAI servers
