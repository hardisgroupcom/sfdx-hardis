...
- WebSocket client: CLI-only commands can now opt out of WebSocket initialization by declaring `public static disableWebsocket = true` on their class. The `init` hook loads the command class and skips the WebSocket client when the flag is set. Applied to [hardis:config:get](https://sfdx-hardis.cloudity.com/hardis/config/get/), [hardis:config:monitoring-defaults](https://sfdx-hardis.cloudity.com/hardis/config/monitoring-defaults/), and [hardis:cache:clear](https://sfdx-hardis.cloudity.com/hardis/cache/clear/).
- [hardis:auth:login](https://sfdx-hardis.cloudity.com/hardis/auth/login/) / [hardis:org:connect](https://sfdx-hardis.cloudity.com/hardis/org/connect/): Fix web login flow - when the browser window is closed or the operation times out before completing, the command now throws a clear error (`authWebLoginNoUserDetected`) instead of silently continuing with null user data.
- [hardis:config:monitoring-defaults](https://sfdx-hardis.cloudity.com/hardis/config/monitoring-defaults/): New read-only command that returns the hardcoded default monitoring and notification configuration (one entry per key with translated title and description, default frequency / scheduling fields, and default per-channel severity thresholds) plus the supported option lists. Designed to power configuration UIs such as the VS Code sfdx-hardis extension; does not read `.sfdx-hardis.yml`.
...
- Disable flavor suggestions in default MegaLinter config
