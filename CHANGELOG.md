...
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): a plan is computed in a few seconds instead of minutes: the git provider is built once per command (each call spawned a git process, eighty at a time during the history walk), the history walk reads 5 comments first and doubles until it finds the last backpromote, the files of the listed merges come from one git call, and a comment read dropped by the provider is tried again instead of ending the plan.
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): the sandbox files retrieved for the comparison are kept in a cache checked against the org's source tracking (or its metadata dates), so a plan computed again on the same window costs one query instead of a retrieve.
