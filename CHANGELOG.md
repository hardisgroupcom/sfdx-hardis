...
- Git providers and ticketing providers: every batch of parallel calls (merged Pull Requests per branch, Deployment Actions comments, Backpromotes comments, JIRA, Azure Boards and ServiceNow ticket details) now leaves 20 at a time and backs off to 10, 5, then one by one when a batch fails, instead of one call at a time or an unbounded burst.
