...
- [hardis:org:monitor:all](https://sfdx-hardis.cloudity.com/hardis/org/monitor/all/)
  - AI-powered monitoring summary: generates a consolidated AI executive summary of all monitoring notifications when an AI provider is configured
  - Weekly PPTX monitoring report: on weekly runs, a PowerPoint report is generated by a coding agent (Claude, Codex, Gemini, or Copilot) and attached to the summary notification
  - New `--force-all` flag to force all commands including weekly ones (equivalent to `MONITORING_IGNORE_FREQUENCY=true`)
- Coding agents
  - `codingAgent` config property is now required (no more auto-detection); locally, agents authenticate via their own login (no API key env var needed)
