...
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): an item is now listed as changed in your org only when it differs from what your org received last, and the merge starts from that same version, so a Pull Request left out is never silently reverted.
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): an org that never received a backpromote is offered the newest Pull Requests only, a Pull Request left out stays in the list, and deployment actions and Apex test classes are read from the parent branch.
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): faster and safer runs, with the delta of a commit cached between steps, no org retrieve when the selection is explicit, and a `--plan` that never changes your current branch.
- [hardis:work:backpromote](https://sfdx-hardis.cloudity.com/hardis/work/backpromote/) (Beta): a git provider that cannot read the Pull Request comments now stops the command instead of offering every Pull Request again, and a history comment is never posted twice on a busy Pull Request.
