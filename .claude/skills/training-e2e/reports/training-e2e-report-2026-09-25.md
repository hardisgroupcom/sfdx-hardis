...
- **F16 (fixed)**: the backpromote fixture, generated by the course's `scripts/build/mocks.mjs`, is
  now the plan of this walk: US-017, US-016 and US-014 by the learner's login, seven items, no
  action. Lab 2.1's summary paragraph promised four lines of actions a learner does not get; it
  reads the summary they get now.
- **F33 (CLI and extension, fixed)**: re-shooting F16 showed that backpromote named a scratch org by
  its org id: "Backpromote to 00dcu…", "Deploying the metadata to 00dcu…", "Keep the 00dcu…
  version", while Lab 2.1 says **Backpromote to helios-dev**. A scratch org has no sandbox name,
  and the CLI puts its org id there. The walk drove that step headlessly and never saw the button.
  The CLI's messages and the panel now use the alias; the branch and the comment rows keep the id.
- **F21 (fixed)**: Lab 2.3's actions list keeps only the backfill, through a capture variant
  (`SFDX_HARDIS_DOC_SCREENSHOTS_ACTIONS_KEEP`); the fixture still holds one action of each type
  for the editor captures.
...
- **Screenshots**: F29 and the Lab 1.2 authorization picture are still stale (see above).
