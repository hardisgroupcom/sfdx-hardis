...
- **DO propagate variables in links**: append `${ds_prom:queryparam}&${ds_loki:queryparam}` (already in the helpers) or `${__all_variables}` for same-dashboard viewPanel links, so navigation keeps org/env/datasource selections.
