...
- **Sibling repositories have their own changelog, and a change made there from here updates it.** `../vscode-sfdx-hardis/CHANGELOG.md` and `../sfdx-hardis-training/CHANGELOG.md` take their entries under `## Unreleased`, in the Pull Request of that repository, with the same style rules. The course has no versions: its `## Unreleased` lines move under a dated heading when they reach `main`.
