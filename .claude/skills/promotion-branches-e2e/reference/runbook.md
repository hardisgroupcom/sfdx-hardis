...
| Restricted promotion steps             | set `allowedPromotionSteps: [{ source: uat, target: preprod }]`, then try `--source-branch integration` and `--target-branch main` | both are refused naming the allowed steps, `--source-branch uat` still works, and the DevOps Pipeline shows **Create promotion** on `uat` only. Remove the key again before the rest of the run                                                            |
