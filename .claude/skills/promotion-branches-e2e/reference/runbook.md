...
| B1b Promotion branch | `git checkout -q -b promotion/integration/uat/2026-09-11-0859`, `e2e_backpromote_json bp-refused-promotion --plan --from "$ROOT"`, then `e2e_backpromote bp-refused-promotion-run --agent --from "$ROOT"`, then back to `feature/E2E-401-dev` and delete the branch | `refused-promotion.json`: plan `blocked`, check `currentBranch` fails naming a promotion branch with the `nextCommand` `sf hardis:work:new --backpromote integration`, no group listed. The run exits 1 with the same message. A promotion branch (and a retrofit branch) only moves User Stories between major branches: it never feeds a developer org |
...
| B1d New User Story receiving a backpromote | from the promotion branch again, `bp_provider_env node "$DEV" hardis:work:new --agent --backpromote integration --task-name "E2E-402 receive backpromote" --target-branch integration --target-org "$DEVORG"` (the command the refused plan of B1b offers, as the panel New User Story button runs it), then `e2e_backpromote_json bp-new-user-story-plan --plan --from "$ROOT"`, then back to `feature/E2E-401-dev`, deleting both branches | exit 0 on `feature/E2E-402-receive-backpromote`, the log says the branch receives the backpromote of `integration` and skips the sandbox update. `new-user-story.json`: `ready` (or `upToDate` when the step is replayed after B4, the scratch org having received every Pull Request), check `currentBranch` passes. In VS Code the command ends with a **Back to backpromote** button reopening the panel on the new branch |
