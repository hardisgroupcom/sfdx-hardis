...
English only for now, and the layout already allows more: the locale is in the path
(`labs/en/...`) from day one, and every translated page carries a `source_rev` front matter key
so staleness is detectable. A translated course means **a new Trailmix per language and per
level**, because a Trailmix cannot be localized.
