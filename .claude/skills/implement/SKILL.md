...
8. **Update CHANGELOG.md**: Add a bullet under `## [beta] (master)`. **Be radically concise: announce the feature in ONE short line per entry, nothing more.** Readers who want details click the command's doc URL. You can delegate this single line to the `changelog` agent (Haiku) via the Agent tool - pass it a short description of the change and the command path if any.
